...
class TestGroupAPIs(TestAPIBaseClass):
    ...
    def test_retrieve_issue_status_and_issues_by_group(self):
        ...
        res3 = self.create_site(groups=["test1"], site="http://foo.com", plans=['test'])
        res4 = self.create_site(groups=["test2"], site="http://bar.com", plans=['test'])
        # if we query just test1, should get back only foo.com
        res5 = self.get_reports_status(user=self.email, group_name="test1")
        r = res5.json()['report']
        print r
        ...
        res6 = self.get_reports_issues(user=self.email, group_name="test1")
        r = res6.json()['report']
        self.assertEqual(len(r), 1) # there should just be one dict returned in the list
        self.assertEqual(r[0]['target'], "http://foo.com")
